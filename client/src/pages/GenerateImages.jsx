...
{
  imageStyle.map((item) => (
   <span onClick={()=> setSelectedStyle(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-green-50 text-green-700' : 'border-gray-300 text-gray-500'}`} key={item}>{item}</span>
  ))}
</div>
...
<button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#00AD25] to-[#04FF50] text-white px-4 py-2 mt-6 rounded-lg text-sm cursor-pointer'>
 {loading ?<ClipLoader size={20} color='white'/>: <Image className='w-5' /> }
  Generate Image
</button>
    </form>
